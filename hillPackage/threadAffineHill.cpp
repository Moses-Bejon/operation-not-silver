...
class AffineHill {
    public:
        AffineHill(int* inp_ciphertext, int len) : ciphertext_len(len) {
        ...
        void keyDictAttack(int test_chunk, int processThreadCount = 1, double notiPer = 80000000, bool maxSpeed = false) {
            ...
            for (int i = 0; i < processThreadCount - 1; i++) {
                ...
                cout << ">>> Process thread: " << i << " for keys " << i * chunkSize * 676 << " to " << (i + 1) * chunkSize * 676 - 1 << " has started\n"; 
            }
            ...
            cout << ">>> Process thread: " << processThreadCount - 1 << " for keys " << start * 676 << " to " << noOfkeys * 676 - 1 << " has started\n"; 
    ...
    private:
        ...
        int* decipher(int len, int inv_key[2][2], int a, int b) {
            int left_val;
            int right_val;
        ...
        void printEncryptionSet(keySet key_set) {
            int* full_plaintext = decipher(ciphertext_len, keys[key_set.key_ref], key_set.a, key_set.b);
        ...
        void printThread(double notiPer) {
            ...
            while (trials.load() < noOfkeys * 676) {
        ...
        void processThread(int start, int subLen) {
            ...
            for (int i = start; i < start + subLen; i++) {
                for (int a = 0; a < 26; a++) {
                    for (int b = 0; b < 26; b++) {
                        int* plaintext = decipher(testChunk, keys[i], a, b);
